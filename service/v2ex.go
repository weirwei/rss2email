...
func V2exService(ctx context.Context) error {
	ilog.Info("V2exService 运行中...")
	defer ilog.Info("V2exService 运行结束...")
	// 从数据库获取订阅信息
	subscriptionList, err := models.NewUserSubscriptionDao().ListBySubscriptionIDAndSubscriptionType(ctx, constants.SubscriptionV2ex, constants.SubscriptionTypeRss)
	if err != nil {
...
func buildV2ex(feed *gofeed.Feed) (subject string, body string) {
	...
	body = fmt.Sprintf(v2exModule, body)
	return
}

var v2exModule = `<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
